...
export default function NotificationBell() {
  ...
  const baseNotifications = [
    {
      id: 1,
      text: "Newly published portfolio site - you're lookin' at it! 🚀",
      read: false,
    },
    {
      id: 2,
      text: "I'm currently learning Astro and React ✨",
      read: false,
    },
  ];

  // Get the ID of the last base notification
  const lastBaseId =
    baseNotifications.length > 0
      ? baseNotifications[baseNotifications.length - 1].id
      : null;

  // Get the ID of the last unread item
  const lastUnreadId =
    notifications.length > 0
      ? [...notifications].reverse().find((n) => !n.read)?.id
      : null;

  // Load from localStorage and merge with the database
  useEffect(() => {
